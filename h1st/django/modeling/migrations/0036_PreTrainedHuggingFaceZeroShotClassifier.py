# Generated by Django 3.2.8 on 2021-10-17 10:06


from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):
    dependencies = [
        ('H1stModel', '0035_PreTrainedHuggingFaceMaskFiller')
    ]

    operations = [
        migrations.CreateModel(
            name='PreTrainedHuggingFaceZeroShotClassifier',

            fields=[
                ('pretrainedhuggingfacetransformer_ptr',
                 models.OneToOneField(
                     auto_created=True,
                     on_delete=django.db.models.deletion.CASCADE,
                     parent_link=True,
                     primary_key=True,
                     related_name=('h1st_pretrained_'
                                   'hugging_face_zero_shot_classifiers'),
                     serialize=False,
                     to='H1stModel.pretrainedhuggingfacetransformer'))
            ],

            options={
                'verbose_name':
                    'Pre-Trained Hugging Face Zero-Shot Classifier',
                'verbose_name_plural':
                    'Pre-Trained Hugging Face Zero-Shot Classifiers',
                'db_table':
                    'H1stModel_PreTrainedHuggingFaceZeroShotClassifier',
                'abstract': False,
                'default_related_name':
                    'h1st_pretrained_hugging_face_zero_shot_classifiers',
                'base_manager_name': 'objects'
            },

            bases=('H1stModel.pretrainedhuggingfacetransformer',)
        )
    ]
